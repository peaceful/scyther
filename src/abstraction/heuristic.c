...
{
  t->seclabel.gb_auth = t->seclabel.prop_auth = t->seclabel.prot_auth =
    UNDEFINED;
  t->seclabel.gb_sec = t->seclabel.prop_sec = t->seclabel.prot_sec =
    UNDEFINED;
  ...
  if (t->seclabel.gb_auth == UNDEFINED)
      ...
      if (t->seclabel.prop_auth == UNDEFINED)
	{
	  if (isSubtermInTermlist (t, auth))
	    ...
	    t->seclabel.prop_auth = NOAUTH;
	}
      t->seclabel.gb_auth =
	min (t->seclabel.prot_auth, t->seclabel.prop_auth);
    }
  ...
  if (essentialForAuth (t) == YES || essentialForSec (t) == YES)
    {
      ...
      int pres = t->seclabel.tmp_auth >= t->seclabel.gb_auth
	&& t->seclabel.tmp_sec >= t->seclabel.gb_sec;
      if (!pres)
	return 0;
    }
  if (realTermEncrypt (t))
...
int
composedTermSecure (Termlist pt)
{
  Termlist tl;
...
buildPatternForHash (int secretOverlap, Termlist * pull, Termlist * keep,
		     int sec_outer, int auth_outer, Term hashfunc)
{
  Termlist tmp = *keep;
  ...
  while (tmp != NULL)
      ...
      else
	{
      ...
      isSafe = (secondOccurrence || need_not_secure) && not_complete_removal
      ...
      else
	{
	  if (cryptotype == SYM)
		...
		buildPatternForEnc (secretOverlap, type, &pull, &op,
				    sec_outer, auth_outer, cryptotype, key);
