		...
		while(roledef!=NULL)
		{
			if(roledef->message!=NULL)
			{
				if(roledef->absMess==NULL)
					...
					if(!isTermEqual(roledef->message,roledef->absMess)) succeed=1;
			}
...
void transferData1(System sys)
{
	...
	while(p!=NULL)
	{
		...
		while(roles!=NULL){
			Roledef roledef = roles->roledef;
			//for each event in the role
			while(roledef!=NULL&&roledef->message!=NULL&&roledef->absMess!=NULL)
			{
				int isClaim = roledef->type==CLAIM;
				roledef->message = roledef->absMess;
				roledef->absMess=NULL;
				if(isClaim) roledef->claiminfo->parameter = roledef->message;
				roledef=roledef->next;
			}
			roles = roles->next;
		}
...
void transferData(System newsys, System sys)
{
	Protocol p = sys->protocols;
	...
	while(p!=NULL)
	{
		...
		while(roles!=NULL){
			...
			while(roledef!=NULL&&roledef->message!=NULL&&roledef->absMess!=NULL)
			{
				...
				newrd->absMess=NULL;
				if(isClaim) newrd->claiminfo->parameter = newrd->message;
				roledef=roledef->next;
				newrd = newrd->next;
			}
			roles = roles->next;
...
void handleNilEvent(System sys)
{
	Protocol p = sys->protocols;
	while(p!=NULL)
	{
		...
		while(roles!=NULL){
			...
			{
				if(isTermEqual(roledef->message, TERM_NIL)&&roledef->type!=CLAIM)
				{
					...
					else*/
					{
						...
						if(isHead) roles->roledef=roledef;
