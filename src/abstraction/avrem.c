...
void extractClearAVFromRole(Role r)
{
	Termlist tl;
	//check for variables
	...
	for(tl=r->declaredconsts;tl!=NULL;tl=tl->next)
	{
		...
		if(isclear) av = termlistAdd(av,tl->term);
	}
}
void extractClearAVFromProt(Protocol p)
{
	Role r = p->roles;
	while(r!=NULL)
	{
...
Term removeAvlist(Term t)
{
	if(realTermEncrypt(t)) return t;
	Termlist tl = av;
	while(tl!=NULL)
	{
...
void extractAtom(Termlist at, Term t){
	if(realTermLeaf(t))
	{
	...
	if(realTermEncrypt(t)){
		if(t->helper.fcall) extractAtom(at,TermOp(t));
		else{
...
Termlist clearInProtMessages(Protocol p)
{
	Termlist tl = av;
	START:
	while(tl!=NULL)
	{
		...
		while(r!=NULL)
		{
			...
			while(rd!=NULL)
			{
				if((rd->type==CLAIM&&rd->message!=NULL&&isSubterm(tl->term,rd->message))||
				  (rd->type!=CLAIM&&!isClearTextinTerm(rd->message,tl->term)))
				{
...
int isSubtermOfSecTerm(Term t, List tl)
{
	while(tl!=NULL)
	{
		Roledef rd = (Roledef)tl->data;
		if(isSubterm(t,rd->message)) return 1;
		tl = tl->next;
	}
	return 0;
}

int isSubtermOfEqTerm(Term t, Eqlist l)
{
	while(l!=NULL)
	{
...
void disjointFromProperties(List secret, Eqlist eqs)
{
	Termlist tl = av;
	while(tl!=NULL)
	{
		if(isSubtermOfSecTerm(tl->term, secret)||isSubtermOfEqTerm(tl->term, eqs))
		{
			if(tl->prev!=NULL) tl->prev->next = tl->next;
			else av = tl->next;
			if(tl->next!=NULL) tl->next->prev=tl->prev;
...
void fixSafenessForAVRemoval(Protocol p)
{
	List evPhi = NULL;
	List evPhiPlus = NULL;
	...
	//condition iii: (P,avrem)-safe
		...
		//subcondition iii
		checkInjectivenessForEvent(removeAvlist,p, evPhiPlus,0,fixIdenticalEvents);
		list_delete(evPhi);
