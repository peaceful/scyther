...
extern Symbol pat;
//term generation
Term createVariableWithType(Symbol s, Term type)
{
...
Term createSimplePatFromType(Term type)
{
	Term t;
...
{
	if(realTermLeaf(t)) t->abst=1;
	else if(realTermEncrypt(t)){
		makeHomomorphic(TermOp(t));
		if(!t->helper.fcall) makeHomomorphic(TermKey(t));
	}
	else{
...
Equation createHomoEquationFromType(Term type)
{
	Equation eq = makeEquation();
	...
	eq->left = left;
	eq->right = left;
	if(realTermLeaf(left)) eq->right->abst=0;
	else
...
Term createTuplePatFromType(Term type)
{
...
Term createPatFromType(Term type)
{
	...
	if(type->helper.fcall)
	{
		return makeTermFcall(createTuplePatFromType(TermOp(type)), TermKey(type));
	}
