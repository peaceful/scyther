...
int isSubtype(Term type1, Term type2){
	...
	if(realTermLeaf(type2))
	{
		...
		{
			if(type1->helper.fcall!=type2->helper.fcall) return false;
			if(type1->helper.fcall) return isSubtype(TermOp(type1), TermOp(type2))&&
										   isTermEqual(TermKey(type1), TermKey(type2));
			return isSubtype(TermKey(type1), TermKey(type2))&&isSubtype(TermOp(type1), TermOp(type2));
		}
	    else return isSubtype(TermOp1(type1), TermOp1(type2))&&isSubtype(TermOp2(type1), TermOp2(type2));
	}
}

Term subTypeInSubterm(Term sttype, Term type)
{
	if(realTermLeaf(type)) return NULL;
	if(isSubtype(sttype,type)) return type;
	...
	else if(realTermTuple(type))
	{
		...
		if(t1) return t1;
		else return subTypeInSubterm(sttype, TermOp2(type));
	}
	return NULL;
}
...
Term getTermType(Term t){
	...
	else if(t->stype!=NULL) return ((Termlist)t->stype)->term;
...
int isComparable(Term typ1, Term typ2)
{
	...
	{
		if(isSubtype(typ1,typ2)) return 1;
		else if(isSubtype(typ2,typ1)) return 1;
		else return 0;
	}
	else //both composed
	{
		if(typ1->type!=typ2->type) return 0;
		if(realTermEncrypt(typ1))
		{
			if(typ1->helper.fcall!=typ2->helper.fcall) return 0;
			else
			{
...
int compareTypelist(Term type, Termlist typelist)
{
	Termlist tl;
...
Term upperCommonSubtype(Term typ1, Term typ2)
{
	if(realTermLeaf(typ1)||realTermLeaf(typ2))
	{
		if(isSubtype(typ1,typ2)) return typ1;
		else if(isSubtype(typ2,typ1)) return typ2;
		else return NULL;
	}
	else //both composed
	{
		if(typ1->type!=typ2->type) return NULL;
		if(realTermEncrypt(typ1))
		{
			if(typ1->helper.fcall!=typ2->helper.fcall) return NULL;
			else
			{
				Term opLeast, keyLeast;
				opLeast = upperCommonSubtype(TermOp(typ1), TermOp(typ2));
				if(!opLeast) return NULL;
				if(typ1->helper.fcall)
				{
					keyLeast= isTermEqual(TermKey(typ1),TermKey(typ2))?TermKey(typ1):NULL;
					if(!keyLeast) return NULL;
					return makeTermFcall(opLeast,keyLeast);
				}
				else
				{
					keyLeast = upperCommonSubtype(TermKey(typ1),TermKey(typ2));
					if(!keyLeast) return NULL;
...
Term mostGeneralType(Term typ1, Term typ2)
{
	if(realTermLeaf(typ1)||realTermLeaf(typ2))
	{
		if(isSubtype(typ1,typ2)) return typ2;
		else if(isSubtype(typ2,typ1)) return typ1;
		else return TERM_Ticket;
	}
	else //both composed
	{
		if(typ1->type!=typ2->type) return NULL;
		if(realTermEncrypt(typ1))
		{
			if(typ1->helper.fcall!=typ2->helper.fcall) return TERM_Ticket;
			else
			{
				Term opGen;
				...
				if(typ1->helper.fcall)
				{
					if(!isTermEqual(TermKey(typ1),TermKey(typ2))) return TERM_Ticket;
